...
export class ReviewService {
  public constructor(app: Application) {
    ...
    app.get('/review/get_all_by_user_test', this.getAllReviewsOfTrailtest);
  }
  ...
  private async getAllReviewsOfTrailtest(req: Request, res: Response) {
    const targetTrailID = parseInt(req.params['trailID']);
    // if (Number.isNaN(targetTrailID)) {
    //   ResponseUtil.respondWithInvalidTrailID(res);
    //   return;
    // }
    // const reviews = await HikEasyApp.Instance.?.find(Review, {
    //   where: [{ trail: targetTrailID }],
    // });
    // const reviewsUser = await getRepository(User).find({relations: ['Review']});
    // const user = await createQueryBuilder("user")
    // .leftJoinAndSelect(Review, "review", "review.trailId = 2")
    // .getMany();
    const user = await getRepository(Review).createQueryBuilder("user")
